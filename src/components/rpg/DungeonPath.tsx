...
function QuestNode({
  l, locked, done, prevNum, side, onClick, delay,
}: {
  l: Lecture;
  locked: boolean;
  done: boolean;
  ...
  const state: "done" | "unlocked" | "locked" = done ? "done" : locked ? "locked" : "unlocked";
  const bg =
    state === "done" ? "linear-gradient(135deg,#fbbf24,#b45309)"
    : state === "unlocked" ? "linear-gradient(135deg,#a78bfa,#6d28d9)"
    : "linear-gradient(135deg,#475569,#1e293b)";
  const glow =
    state === "done" ? "rgba(251,191,36,0.75)"
  ...
  return (
    ...
    >
      <button type="button" onClick={onClick} className="relative shrink-0 focus:outline-none group">
        <motion.div
        ...
        >
          {state === "done" ? <CheckCircle2 className="h-7 w-7" />
      ...
      >
        <div className="text-[10px] uppercase tracking-widest text-amber-300 font-bold">
          {state === "done" ? "Cleared" : state === "locked" ? "Sealed" : "Ready"}
        </div>
        <div className="font-extrabold truncate text-amber-50">{l.lecture_title}</div>
        <div className="text-xs text-muted-foreground mt-1 truncate">
          {locked
            ? `🔒 Finish Quest ${String(prevNum ?? "").padStart(2, "0")} to break the chain`
            : done
            ? "Quest completed"
            : "Watch to earn XP"}
        </div>
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        defeated ? "border-amber-400/60"
          : ready ? "border-rose-500/60 cursor-pointer"
    ...
    >
      <div className="relative flex items-center gap-3">
        <div className="h-16 w-16 md:h-20 md:w-20 rounded-2xl grid place-items-center shrink-0"
          style={{
            background: defeated ? "linear-gradient(135deg,#f59e0b,#b45309)"
              : ready ? "linear-gradient(135deg,#ef4444,#7f1d1d)"
              : "linear-gradient(135deg,#475569,#1e293b)",
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-muted-foreground">
            {defeated ? `Cleared · +${xp} XP · +${coins} coins`
              : ready ? `Reward: +${xp} XP · +${coins} coins`
